#!/usr/bin/env bash
#
# 로컬 DB 에 duo_crew 계정과 crew_svc 스키마를 만든다.
#
...
# 시크릿 위치.
#
# 분리된 레포에서는 이 레포 루트($REPO)에 사본이 있다 — 다른 서비스와 같다.
# 다만 crew 는 backend 모노레포에도 그대로 남아 있어서, 거기서 돌릴 때는
# backend/crew/../application-secret.yml 이 실제 위치다. 둘 다 본다.
SECRET=""
for candidate in "$REPO/application-secret.yml" "$REPO/../application-secret.yml"; do
  [ -f "$candidate" ] && { SECRET="$candidate"; break; }
done

[ -n "$SECRET" ] || {
  echo "application-secret.yml 이 없습니다: $REPO/application-secret.yml" >&2
