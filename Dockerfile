...
#     docker build -f crew/Dockerfile -t gamehouse:crew .
#
# 이유: 이 모듈은 :common 을 의존한다. 컨텍스트를 crew/ 으로 잡으면
...
RUN ./gradlew --no-daemon :crew:dependencies

COPY common/src ./common/src
COPY crew/src ./crew/src
RUN ./gradlew --no-daemon :crew:bootJar
RUN cp crew/build/libs/*.jar /app/app.jar && mkdir -p /out/uploads
...
# 볼륨이 소유권을 상속받아 쓰기 가능하도록 spring 소유로 준비 (crew는 지금 업로드 파일이 없지만
